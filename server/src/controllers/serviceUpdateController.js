...
export const getServiceUpdates = async (req, res) => {
    try {
        ...
        const serviceUpdates = await serviceUpdateModel.find(query)
            .populate({
                path: 'serviceBooking',
                populate: [
...
export const createServiceUpdate = async (req, res) => {
    try {
        const { serviceBooking, description, mediaUrls, requiresApproval } = req.body;
...
export const updateServiceUpdate = async (req, res) => {
    try {
        ...
        const serviceUpdate = await serviceUpdateModel.findByIdAndUpdate(
            id,
            updateData,
            { new: true }
        ).populate({
...
export const updateApprovalStatus = async (req, res) => {
    try {
        ...
        const updatedServiceUpdate = await serviceUpdateModel.findByIdAndUpdate(
            id,
        ...
        res.status(200).json({
            success: true,
            message: `Service update ${approved ? 'approved' : 'rejected'} successfully`,
...
export const getServiceUpdatesByBooking = async (req, res) => {
    try {
        ...
        const serviceUpdates = await serviceUpdateModel.find({ serviceBooking: bookingId })
            .populate({
                path: 'serviceBooking',
                populate: [
                    { path: 'user', select: 'username email' },
...
export const getPendingApprovals = async (req, res) => {
    try {
        const serviceUpdates = await serviceUpdateModel.find({
