...
export const getServiceBookingById = async (req, res) => {
  try {
    const { id } = req.params;
    const booking = await serviceBookingModel
      .findById(id)
      .populate("user", "username email")
      .populate("bike", "model brand registrationNumber")
      .populate("servicePackage", "name description price")
...
export const updateServiceBooking = async (req, res) => {
  try {
    ...
    if (
      updateData.serviceSlot &&
      updateData.serviceSlot !== existingBooking.serviceSlot.toString()
    ) {
    ...
    const updatedBooking = await serviceBookingModel
